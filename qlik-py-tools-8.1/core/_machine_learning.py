...
class PersistentModel:
    ...
    def __init__(self):
        """Basic contructor""" 
        self.name, self.state, self.state_timestamp, self.using_keras = None, None, None, False     
    def save(self, name, path, overwrite=True, compress=3, locked_timeout=2):
        ...
        try:
            ...
            pass 
        if Path(f).exists() and not overwrite:  # If the file exists and overwriting is not allowed, raise an exception
        ...
        elif Path(f_lock).exists(): # Check if the file is currently locked
            time.sleep(locked_timeout) # Wait a few seconds and check again            
            if Path(f_lock).exists():   # If the file is still locked raise an exception
            ...
            self.name, self.state, self.state_timestamp = name, 'saved', time.time()
            try:    # Keras models are excluded from the joblib file as they are saved to a special HDF5 file in _sklearn.py
                if self.using_keras: # Get the trained keras model from the pipeline's estimator    
                    keras_model = self.pipe.named_steps['estimator'].model   
                    keras_model.save(path + name + '.h5', overwrite=overwrite)  # Save the keras model architecture and weights to disk                                 
                    self.pipe.named_steps['estimator'].model = None # The Keras estimator is excluded from the model saved to the joblib file
            except AttributeError:
                pass    
            joblib.dump(f_lock, filename=Path(f_lock), compress=compress) # Create the lock file
            try:
                joblib.dump(self, filename=Path(f), compress=compress) # Store this instance to file
            finally: 
                Path(f_lock).unlink() # Delete the lock file
        return self
...
class Preprocessor(TransformerMixin):
    ...
    """
    def __init__(self, features, return_type='np', scale_hashed=True, scale_vectors=True, 
        missing="zeros", scaler="StandardScaler", logfile=None, **kwargs):
        """
    ...
    def fit(self, X, y=None, features=None, retrain=False):
        ...
        if retrain:        # Reinitialize this Preprocessor instance if required
            if features is None:
                features = self.features
            self.__init__(features)        
        scale_df = pd.DataFrame() # Set up an empty data frame for data to be scaled
        ohe_df = None
        hash_df = None
        cv_df = None
        tfidf_df = None
        text_df = None
        if self.ohe:
            ohe_df = X[self.ohe_meta.index.tolist()] # Get a subset of the data that requires one hot encoding
            ohe_df = pd.get_dummies(ohe_df, columns=ohe_df.columns) # Apply one hot encoding to relevant columns
            self.ohe_df_structure = pd.DataFrame().reindex_like(ohe_df) # Keep a copy of the OHE dataframe structure so we can align the transform dataset 
        if self.scale:# Scaling needs to be fit exclusively on the training data so as not to influence the results
    ...
    def transform(self, X, y=None):
        ...
        if self.hash and self.scale_hashed:
            if self.scale:
                ...
                self.scaler_instance = utils.get_scaler(scale_df, missing=self.missing, scaler=self.scaler, 
                    **self.kwargs)
        elif self.hash:
        ...
        try:
            ...
            if len(scale_df) > 0:
                ...
                scale_df = pd.DataFrame(self.scaler_instance.transform(scale_df), 
                            index=scale_df.index, columns=scale_df.columns)
    ...
    def text_similarity(df, col):
        """
        Convert strings to their unicode representation and then apply one hot encoding, 
        creating one feature for each unique character in the column. 
        ...
        encoded = pd.DataFrame(mlb.fit_transform(encoded[col]),
            columns=mlb.classes_, index=encoded.index).add_prefix(col+"_")
...
class TargetTransformer:
    ...
    It can also make the series stationary by differencing the values. 
    Note that this is only valid when predictions will include multiple samples.
    An inverse transform method allows for reversing the transformations.
    """

    def __init__(self, scale=True, make_stationary=None, stationarity_lags=[1], 
        missing="zeros", scaler="StandardScaler", logfile=None, **kwargs):
        """Initialize the TargetTransformer instance.
        scale is a boolean parameter to determine if the target will be scaled.
        make_stationary is a parameter to determine if the target will be made stationary. 
        This should only be used for sequential data.
        Passing make_stationary='log' will apply a logarithm to the target and use an exponential 
            for the inverse transform.
        Passing make_stationary='difference' will difference the values to make the target series stationary.
        By default the difference will be done with lag = 1. Alternate lags can be provided by 
        passing a list using the stationarity_lags parameter.
    ...
    def fit(self, y):
        """Make the data stationary if required. Fit the scaler to target values from the training set.
        """
...
class Reshaper(TransformerMixin):
    ...
    def __init__(self, first_layer_kwargs=None, logfile=None, **kwargs):
        ...
        first_layer_kwargs should be a reference to the first layer kwargs of the Keras architecture 
    ...
    def transform(self, X, y=None):
        ...
        # e.g. The input_shape can be a tuple of (samples, subsequences, timesteps, features),
        #  with subsequences and timesteps as optional.
...
class KerasClassifierForQlik(KerasClassifier):
    ...
    This class takes in a compiled Keras model as part of sk_params and uses 
    the __call__ method as the default build_fn.
...
class KerasRegressorForQlik(KerasRegressor):
    ...
    This class takes in a compiled Keras model as part of sk_params and uses the __call__ method 
    as the default build_fn.
    It also stores a histories dataframe to provide metrics for each time the model is fit.
    """
